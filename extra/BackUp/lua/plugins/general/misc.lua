local plugins_path = vim.fn.stdpath('data')..'/site/pack/paqs/start/'
...
if vim.fn.empty(vim.fn.glob(plugins_path..'vim-easy-align')) == 0 then
  ...
  xnmap('ga' , '<Plug>(EasyAlign' , {noremap=false})

  -- Start interactive EasyAlign for a motion/text object (e.g. gaip)
  nnmap('ga','<Plug>(EasyAlign',{noremap=false})
...
if vim.fn.empty(vim.fn.glob(plugins_path..'vim-better-whitespace')) == 0 then
  ...
  nnmap( '<leader>ws', ':ToggleWhitespace<CR>')
  nnmap( '<leader>wr', ':StripWhitespace<CR>')
  nnmap( '<leader>wt', ':ToggleStripWhitespaceOnSave<CR>')
...
if vim.fn.empty(vim.fn.glob(plugins_path..'FastFold')) == 0 then
  ...
  -- vim.api.nvim_set_keymap('n','<SID>(DisableFastFoldUpdate)', '<Plug>(FastFoldUpdate)', {noremap=false})
